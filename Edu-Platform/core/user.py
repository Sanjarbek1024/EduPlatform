...
class Student(User):
    ...
    def submit_assignment(self, assignment_id, content):
        if assignment_id in self.assignments and self.assignments[assignment_id] == 'submitted':
            print(f"\n Assignment {assignment_id} already submitted.")
            return

        self.assignments[assignment_id] = 'submitted'
        print(f"\n Assignment {assignment_id} submitted with content: {content}")
        ...
        if assignment:
            ...
            print(f"\n Assignment {assignment_id} not found in system.")
...
class Teacher(User):
    ...
    def grade_assignment(self, assignment_id, student_id, grade):
        if assignment_id in self.assignments:
            ...
            print(f"\n Student {student_id} graded {grade} for assignment {assignment_id}")
        else:
...
class Admin(User):
    ...
    def add_user(self, user):
        users.append(user)
        print(f"\n User '{user._full_name}' added with role {user.role.value}")

    def remove_user(self, user_id):
        global users
        users = [u for u in users if u._id != user_id]
        print(f"\n User with ID {user_id} removed.")
